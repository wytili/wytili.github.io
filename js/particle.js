/*本文件源自https://github.com/hustcc/canvas-nest.js/*/
!function() {
    "use strict";
    function e(e) {
    ...
    var y = function() {
        ...
        },
        {
            ...
            value: function() {
                ...
                t.style.cssText = "display: block; position: fixed; top: 0; left: 0; width: 100%; height: 100%; z-index: " + this.c.zIndex + "; opacity: " + this.c.opacity;
            ...
            value: function() {
                ...
                r.forEach(function(r, f) {
                    for (r.x += r.xa, r.y += r.ya, r.xa *= r.x > n || r.x < 0 ? -1 : 1, r
                        .ya *= r.y > o || r.y < 0 ? -1 : 1, t.fillStyle = "rgba(" + e.c.pointColor + ")",
                             t.fillRect(r.x - .5, r.y - .5, 1, 1), u = f + 1; u < a.length; u++) null !== (s = a[u]).x
                    && null !== s.y && (l = r.x - s.x, d = r.y - s.y, (v = l * l + d * d) < s.max &&
                    (s === i && v >= s.max / 2 && (r.x -= .03 * l, r.y -= .03 * d), c = (s.max - v) / s.max, t.beginPath(),
                        t.lineWidth = c / 2, t.strokeStyle = "rgba(" + e.c.color + "," + (c + .2) + ")", t.moveTo(r.x, r.y),
                        t.lineTo(s.x, s.y), t.stroke()))
                }),
                this.requestFrame(this.drawCanvas)
            }
        },
